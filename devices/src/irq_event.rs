...
pub struct IrqEdgeEvent(EventFd);

impl IrqEdgeEvent {
    pub fn new() -> Result<IrqEdgeEvent> {
        Ok(EventFd::new(EFD_NONBLOCK).map(IrqEdgeEvent)?)
    }

    pub fn try_clone(&self) -> Result<IrqEdgeEvent> {
        Ok(self.0.try_clone().map(IrqEdgeEvent)?)
    }

    /// Creates an instance of IrqLevelEvent from an existing event.
    pub fn from_event(trigger_evt: EventFd) -> IrqEdgeEvent {
        IrqEdgeEvent(trigger_evt)
    }

    pub fn get_trigger(&self) -> &EventFd {
        &self.0
    }
...
impl IrqLevelEvent {
    pub fn new() -> Result<IrqLevelEvent> {
        let trigger_evt = EventFd::new(EFD_NONBLOCK)?;
        let resample_evt = EventFd::new(EFD_NONBLOCK)?;
        Ok(IrqLevelEvent {
            trigger_evt,
            resample_evt,
        })
    ...
    pub fn from_event_pair(trigger_evt: EventFd, resample_evt: EventFd) -> IrqLevelEvent {
        IrqLevelEvent {
            trigger_evt,
            resample_evt,
        }
    ...
    pub fn get_resample(&self) -> &EventFd {
        &self.resample_evt
    }

    /// Allows backend to inject interrupt (typically into guest).
    pub fn trigger(&self) -> Result<()> {
        Ok(self.trigger_evt.write(1)?)
    }
...
impl AsRawFds for IrqEdgeEvent {
    fn as_raw_fds(&self) -> Vec<RawFd> {
        vec![self.0.as_raw_fd()]
    }
}

impl AsRawFds for IrqLevelEvent {
    fn as_raw_fds(&self) -> Vec<RawFd> {
        vec![self.trigger_evt.as_raw_fd(), self.resample_evt.as_raw_fd()]
    }
}
